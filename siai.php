...
$touhyou_date = date("Y-m-d H:i:s");
$stmt7 = $pdo3->prepare("SELECT * FROM $table_name2 WHERE touhyou_name = :touhyou_name");
$stmt7->bindParam(':touhyou_name',$touhyou_name, PDO::PARAM_STR);
$res7 = $stmt7->execute();
if($res7){
$touhyou_id3 = $stmt23->fetchall();
$cnt5 = count($touhyou_id3);
}
...
if(!empty($_GET['btn_submit'])){
  ...
  if(!empty($_GET['bluegreen2'])){
    $stmt11 = $pdo3->prepare("INSERT INTO $table_name2 ( touhyou_name, ikena_hyousuu, ikenb_hyousuu, touhyou_date, ikena_tokutenn, ikenb_tokutenn) VALUES ( :touhyou_name, :ikena_hyousuu, :ikenb_hyousuu, :touhyou_date, :ikena_tokutenn, :ikenb_tokutenn)");
    $stmt11->bindParam(':touhyou_name', $touhyou_name, PDO::PARAM_STR);
    $stmt11->bindParam(':ikena_hyousuu', $ikena_hyousuu, PDO::PARAM_INT);
    $stmt11->bindParam(':ikenb_hyousuu', $ikenb_hyousuu, PDO::PARAM_INT);
    $stmt11->bindParam(':touhyou_date', $touhyou_date, PDO::PARAM_STR);
    $stmt11->bindParam(':ikena_tokutenn', $ikena_tokutenn, PDO::PARAM_INT);
    $stmt11->bindParam(':ikenb_tokutenn', $ikenb_tokutenn, PDO::PARAM_INT);
    $res11 = $stmt11->execute();
    $stmt15 = $pdo3->prepare("SELECT * FROM $table_name2 WHERE touhyou_name = :touhyou_name");
    $stmt15->bindParam(':touhyou_name',$touhyou_name, PDO::PARAM_STR);
    $res15 = $stmt15->execute();
    if($res15){
      $touhyou_id1 = $stmt15->fetchall();
      $cnt2 = count($touhyou_id1);
      $touhyou_id2 = $touhyou_id1[0]['touhyou_id'];
    }
    if($cnt2>1){
      $stmt16 = $pdo3->prepare("DELETE FROM $table_name2 WHERE touhyou_id = $touhyou_id2");
...
if( !empty($_GET['btn_submit']) ) {
  ...
  if( empty($error_message) ) {
    ...
    try {
    $stmt17 = $pdo2->prepare("INSERT INTO $table_name1 (color,comment_name, comment, comment_datetime, hensinid, hensinname, hensincolor) VALUES ( :color,:comment_name, :comment, :comment_datetime, :hensinid, :hensinname, :hensincolor)");
    $stmt17->bindParam( ':color', $color1, PDO::PARAM_STR);
    $stmt17->bindParam( ':comment_name', $view_name, PDO::PARAM_STR);
    $stmt17->bindParam( ':comment', $message, PDO::PARAM_STR);
    $stmt17->bindParam( ':comment_datetime', $current_date, PDO::PARAM_STR);
  ...
  $stmt18 = $pdo1->prepare("SELECT * FROM gidai WHERE gidai_id = :gidai_id");
  $stmt18->bindParam( ':gidai_id', $last_id, PDO::PARAM_INT);
  $res18 = $stmt18->execute();
  if($res18){
    $gidai_date = $stmt18->fetch();
    $etsurannsuu = $gidai_date['etsurannsuu'] + 1;
  }
  if( empty($error_message) ) {
    $stmt19 = $pdo1->prepare("UPDATE gidai SET etsurannsuu = :etsurannsuu WHERE gidai_id = :gidai_id");
    $stmt19->bindParam( ':gidai_id', $last_id, PDO::PARAM_INT);
    $stmt19->bindParam( ':etsurannsuu', $etsurannsuu, PDO::PARAM_INT);
    $res19 = $stmt19->execute();
  }
  }
  }
  }
  $stmt20 = $pdo2->prepare("SELECT MAX(comment_id) FROM $table_name1");
  $res20 = $stmt20->execute();
  if($res20){
   $maxcomment_data = $stmt20->fetch();
   $cnt6 = $maxcomment_data['MAX(comment_id)'];
  }
  $stmt21 = $pdo1->prepare("UPDATE gidai SET cnt=:cnt WHERE gidai_id = $last_id;");
  $stmt21->bindParam(':cnt', $cnt6, PDO::PARAM_INT);
  $res21=$stmt21->execute();
  if(!empty($_GET['comment_idhensin'])){
    $comment_idhensin = $_GET['comment_idhensin'];
    $stmt22 = $pdo2->prepare("SELECT * FROM $table_name1 WHERE comment_id = :comment_id");
    $stmt22->bindParam(':comment_id',$comment_idhensin,PDO::PARAM_INT);
    $res22=$stmt22->execute();
    if($res22){
      $comment_data1 = $stmt22->fetch();
      $hensinid = $comment_data1['comment_id'];
      $hensinname = $comment_data1['comment_name'];
      $hensincolor = $comment_data1['color'];
    }
    $stmt23 = $pdo2->prepare("UPDATE $table_name1 SET hensinid=:hensinid, hensinname=:hensinname, hensincolor=:hensincolor WHERE comment_id = $cnt6;");
    $stmt23->bindParam(':hensinid', $hensinid, PDO::PARAM_INT);
    $stmt23->bindParam(':hensinname', $hensinname, PDO::PARAM_STR);
    $stmt23->bindParam(':hensincolor', $hensincolor, PDO::PARAM_STR);
    $res23=$stmt23->execute();
  }
  $stmt24 = $pdo3->prepare("select sum(ikena_hyousuu),sum(ikenb_hyousuu) from $table_name2;");
  $res24 = $stmt24->execute();
  if($res24){
    $touhyou_data = $stmt24->fetch();
    $ikena_tokutenn = $touhyou_data['sum(ikena_hyousuu)'];
    $ikenb_tokutenn = $touhyou_data['sum(ikenb_hyousuu)'];
  }
  $stmt25 = $pdo3->prepare("SELECT MAX(touhyou_id) FROM $table_name2");
  $res25 = $stmt25->execute();
  if($res25){
   $maxtouhyou_data = $stmt25->fetch();
   $maxtouhyou_id = $maxtouhyou_data['MAX(touhyou_id)'];
  }
  $stmt26 = $pdo3->prepare("UPDATE $table_name2 SET ikena_tokutenn=:ikena_tokutenn,ikenb_tokutenn=:ikenb_tokutenn WHERE touhyou_id = $maxtouhyou_id;");
  $stmt26->bindParam(':ikena_tokutenn', $ikena_tokutenn, PDO::PARAM_INT);
  $stmt26->bindParam(':ikenb_tokutenn', $ikenb_tokutenn, PDO::PARAM_INT);
  $res26=$stmt26->execute();
  $stmt27 = $pdo1->prepare("UPDATE gidai SET ikenA_tokutenn=:ikenA_tokutenn,ikenB_tokutenn=:ikenB_tokutenn WHERE gidai_id = $last_id;");
  $stmt27->bindParam(':ikenA_tokutenn', $ikena_tokutenn, PDO::PARAM_INT);
  $stmt27->bindParam(':ikenB_tokutenn', $ikenb_tokutenn, PDO::PARAM_INT);
  $res27=$stmt27->execute();
  $stmt28 = $pdo1->prepare("SELECT * FROM gidai WHERE gidai_id = :gidai_id");
  $stmt28->bindParam( ':gidai_id', $last_id, PDO::PARAM_INT);
  $res28 = $stmt28->execute();
  if( $res28 ) {
   $gidai_data = $stmt28->fetch();
  ...
  $stmt29 = $pdo1->prepare("UPDATE gidai SET keltsutyaku=:keltsutyaku WHERE gidai_id = $last_id;");
  $stmt29->bindParam(':keltsutyaku', $touhyou, PDO::PARAM_STR);
  $res29=$stmt29->execute();
...
$stmt30 = $pdo2->prepare("SELECT * FROM $table_name1");
$res30= $stmt30->execute();
if($res30){
  $comment_data = $stmt30->fetchall();
  $cnt1 = count($comment_data);
}
...
<html>
  <head　prefix=”og: http://ogp.me/ns# fb: http://ogp.me/ns/fb# article: http://ogp.me/ns/article#”>
    ...
    <meta property="og:url" content="https://netkousienn.com/ranking.php?siai_id=<?php echo $last_id?>" />
             ...
             <?php if(!empty($syouri_messageA)): ?>
              ...
              <?php if(!empty($syouri_messageB)): ?>
               ...
               <?php endif; ?>
                 ...
                 <?php if( !empty($color1) && $touhyou==='ari' ): ?>
